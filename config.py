class Config:
	def __init__(self):
		self.domain = "code-runner-plugin-tau.vercel.app" # 部署CodeRunner-Plugin的域名或ip,要求协议完整eg.https://example.com or http://example.com
		self.proxydomain = "code-runner-plugin-tau.vercel.app" # CodeRunner-Plugin 的代理服务器域名或ip，由于vercel域名或ip可能被墙，代码或图片在国内无法下载，若不需要代理则与self.domain内容相同
